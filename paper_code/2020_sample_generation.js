...
var CDL_2020=ee.ImageCollection("USDA/NASS/CDL").filterDate('2020-01-01','2020-12-31').first().select('cropland').clip(studyArea)
//maize
var maize_2020=CDL_2020.eq(1).selfMask()
Map.addLayer(maize_2020,{min:1,max:1,palette:['ffd300']},'maize_2020')
//soybean
var soybean_2020=CDL_2020.eq(5).selfMask()
Map.addLayer(soybean_2020,{min:1,max:1,palette:['267000']},'soybean_2020')
//othercrop
var maize_soybean_2020=ee.ImageCollection.fromImages([maize_2020,soybean_2020]).mosaic().rename('label_1').unmask(0).updateMask(Cropland)
var othercrop_2020=maize_soybean_2020.eq(0).selfMask()
Map.addLayer(othercrop_2020,{min:1,max:1,palette:['00a8e2']},'othercrop_2020')
/**************************************************************************
...
***************************************************************************/
      ...
      var NDVI_July=S2.filterDate('2020-07-01','2020-08-01')
      ...
      for(var i=0;i<=10;i++){
        ...
        var maize_mask=neighFun(maize_2020,i,studyArea)
        var inward_maize=maize_2020.updateMask(maize_mask).int16().selfMask()
      ...
      for(var i=0;i<=10;i++){
        ...
        var soybean_mask=neighFun(soybean_2020,i,studyArea)
        var inward_soybean=soybean_2020.updateMask(soybean_mask).int16().selfMask()
...
var maize_mask=neighFun(maize_2020,1,studyArea)
var inward_maize=maize_2020.updateMask(maize_mask).int16().selfMask().rename('Label')

var soybean_mask=neighFun(soybean_2020,1,studyArea)
var inward_soybean=soybean_2020.updateMask(soybean_mask).int16().selfMask().multiply(2).rename('Label')

var othercrop_mask=neighFun(othercrop_2020,0,studyArea)
var inward_othercrop=othercrop_2020.updateMask(othercrop_mask).int16().selfMask().multiply(0).rename('Label')
...
// *********************************************************************************/
      ...
      var year = 2020;
      var startDate = ee.Date('2020-05-01'),endDate = ee.Date('2020-10-01'),SY = ee.Date('2020-01-01');
      var interval = 30;
...
var sample_list=ee.List([maizesample_generate_2020_1,soybeansample_generate_2020_1,othercropsample_generate_2020_1])
...
var purify_sample_list=sample_list.map(function(sample){
  ...
  var MeanStdDevPlus=ee.List.sequence(0,5).map(function(item){
    var MeanStdDevPlus = ee.Number(Mean.get(item)).add(StdDev.get(item)).add(StdDev.get(item)).add(StdDev.get(item));
    return MeanStdDevPlus;
  })
  
  var MeanStdDevMinus = ee.List.sequence(0,5).map(function(item){
    var MeanStdDevMinus = ee.Number(Mean.get(item)).subtract(StdDev.get(item)).subtract(StdDev.get(item)).subtract(StdDev.get(item));
...
*********************************************************************************/
      ...
      var maizesample_purify_2020_withfeature=featureImg.sampleRegions({
      ...
      var soybeansample_purify_2020_withfeature=featureImg.sampleRegions({
      ...
      var othercropsample_purify_2020_withfeature=featureImg.sampleRegions({
